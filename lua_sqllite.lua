...
-- Generate rooms
local room_stmt =
	db:prepare("INSERT INTO rooms (room_number, building_name, floor_number, room_type, capacity) VALUES (?,?,?,?,?)")
...
-- Copy rooms
local file_room_stmt = file_db:prepare(
	"INSERT INTO rooms (id, room_number, building_name, floor_number, room_type, capacity) VALUES (?,?,?,?,?,?)"
)
for row in db:nrows("SELECT * FROM rooms") do
	file_room_stmt:bind_values(
		row.id,
		row.room_number,
		row.building_name,
		row.floor_number,
...
os.execute(
	string.format(
		"sqlite3 %s \"SELECT 'Rooms:', COUNT(*) FROM rooms; SELECT 'Sensor logs:', COUNT(*) FROM sensor_logs;\"",
		DB_FILE
	)
)
...
print("\n=== Performance Test ===")
os.execute(
	string.format('sqlite3 %s ".timer on" "SELECT COUNT(*) FROM sensor_logs WHERE temperature_celsius > 40;"', DB_FILE)
)
